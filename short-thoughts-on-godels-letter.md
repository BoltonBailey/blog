...
> If there really were a (Turing) machine (deciding the existence of length n proofs of formulas of first order predicate logic) with (runtime) ∼ k ⋅ n (or even ∼ k ⋅ n^2), this would have consequences of the greatest importance.
...
If the ∼ k⋅n^2 proof search program can be run for $n$ steps, it would only require 4x the resources to run for $2n$ steps.
